# train_milk_fusion.py
# -*- coding: utf-8 -*-
"""
修改自 train_beef_3class_fusion.py
适配 milk_data.xlsx：
  - 自动提取分量和频率（列名格式 分量_频率）
  - 模型输入维度动态化
  - 输出目录 run/milk_ours
  - 修复保存伪图时的坐标轴错误（转置原始数据）
新增：
  - 平均推理时间测量（每折验证集）
  - 模型大小（参数数量、参数内存、模型文件大小）
修正：
  - 添加缺失的 save_normalmap_with_axes 函数定义

仅修改：
  - 伪图颜色映射，改为蓝-青-浅蓝-米金-棕金色系
  - 其余代码逻辑保持不变
"""
...
# ----------------------------
# 1) 读 Excel -> [N, n_comp, n_freq]
# ----------------------------
def parse_columns_and_build_matrix(df: pd.DataFrame,
                                   label_col: str = "kind"):
    """
    从Excel列名中自动提取分量和频率。
    列名格式如：CP_60, CS_80, Z_5200, Φ_10000, ...
    返回：
      X: float32 [N, n_comp, n_freq]
      y: int64   [N]
      comps: list of str  （按出现顺序排序）
      freqs: list of int   （按数字升序）
    ...
    pat = re.compile(r"^(.+?)[_]\s*(\d+)\s*$")   # 匹配 "CP_60" 格式
    colmap = {}
    comp_set = set()
    freq_set = set()
    ...
    # 排序
    comps = sorted(comp_set)                    # 按字母顺序
    freqs = sorted(freq_set)                    # 数字升序

    # 检查是否所有 (comp,freq) 都存在
    missing = []
    for comp in comps:
        for f in freqs:
    ...
    return X, y, comps, freqs, colmap


class MilkDataset(Dataset):
    def __init__(self, X, y, raw_X_for_save=None):
        self.X = torch.tensor(X, dtype=torch.float32)      # [N, n_comp, n_freq] (标准化后)
        self.y = torch.tensor(y, dtype=torch.long)         # [N]
        self.raw = None
        if raw_X_for_save is not None:
            self.raw = torch.tensor(raw_X_for_save, dtype=torch.float32)  # [N, n_comp, n_freq] (原始)
...
# ----------------------------
# 2) 模型模块：每分量1D小卷积（groups = n_components）
# ----------------------------
class PerComponentConv(nn.Module):
    """
    输入:  [B, n_comp, n_freq]
    输出:  [B, n_comp, n_freq]
    每个分量(通道)独立做1D卷积（groups = n_comp）
    """
    def __init__(self, n_components, hidden_per_comp=8, k=3):
        super().__init__()
        self.n_comp = n_components
        self.conv_expand = nn.Conv1d(
            in_channels=n_components,
            out_channels=n_components * hidden_per_comp,
            kernel_size=k,
            padding=k // 2,
            groups=n_components,
            bias=False
        )
        self.bn1 = nn.BatchNorm1d(n_components * hidden_per_comp)

        self.conv_reduce = nn.Conv1d(
            in_channels=n_components * hidden_per_comp,
            out_channels=n_components,
            kernel_size=1,
            groups=n_components,
            bias=False
        )
        self.bn2 = nn.BatchNorm1d(n_components)
...
# ----------------------------
# 4) 伪图生成器：height map [B,1, n_freq, n_comp] -> 框架图色系 pseudo image
# ----------------------------
class PseudoImageGenerator(nn.Module):
    """
    输入:  height map [B,1, H, W]   H = n_freq, W = n_comp
    输出:  pseudo image [B,3,S,S]

    仅修改配色，不改整体结构：
      - 先用高度值映射到 蓝-青-浅蓝-米金-棕金 色系
      - 再结合坡度做轻微明暗调制
      - 保留上采样和 refine block
    """
    ...
    def forward(self, h):
        ...
        h_norm = (h - h_min) / (h_max - h_min + 1e-6)   # [B,1,H,W]

        dzdy = h[:, :, 2:, :] - h[:, :, :-2, :]         # [B,1,H-2,W]
        dzdx = h[:, :, :, 2:] - h[:, :, :, :-2]         # [B,1,H,W-2]
        ...
        slope_norm = slope / (slope_max + 1e-6)         # [B,1,H,W]

        # 1) 框架图色系映射
        base = palette_map_torch(h_norm)                # [B,3,H,W]

        # 2) 坡度轻微明暗调节，让伪图更自然
        shade = 0.82 + 0.18 * (1.0 - slope_norm)        # [B,1,H,W]
...
class MilkNet(nn.Module):
    def __init__(self,
        ...
        self.per_comp = PerComponentConv(n_components=n_components, hidden_per_comp=8, k=3)
        ...
        # --- 原谱数据分支（输入 n_comp * n_freqs -> spec_feat_dim）
        self.spec_encoder = nn.Sequential(
            nn.Flatten(),                # [B, n_comp * n_freqs]
            nn.Linear(n_components * n_freqs, 128),
            nn.ReLU(inplace=True),
    ...
    def forward(self, x):
        """
        x: [B, n_comp, n_freq] (标准化后的原谱数据)
        return:
          logits: [B, num_classes]
          img:    [B,3,S,S] (伪图)
          h:      [B,1, n_freq, n_comp] (高度图)
        """
        x_in = x

        # 伪图分支
        x_proc = self.per_comp(x_in)                          # [B, n_comp, n_freq]
        h = x_proc.transpose(1, 2).unsqueeze(1)               # [B,1, n_freq, n_comp]
...
# 7) 可视化工具
# ----------------------------
def height_to_normalmap_np(height: np.ndarray):
    """
    height: [H, W] (H = n_freq, W = n_comp)
    return: pseudo color RGB in [0,1], shape [H, W, 3]
    ...
    base = palette_map_np(h_norm)                    # [H,W,3]
    shade = 0.82 + 0.18 * (1.0 - slope_norm)         # [H,W]
    img = base * shade[..., None]
...
def save_surface_3d(height: np.ndarray, save_path: Path,
                    comps, freqs):
...
def save_normalmap_with_axes(nm_rgb: np.ndarray,
                             save_path: Path,
                             comps, freqs,
                             dpi=200):
    """
...
def train_one_fold(fold_id: int,
    ...
    X_train_s = scaler.fit_transform(X_train_flat).reshape(-1, len(comps), len(freqs)).astype(np.float32)
    X_val_s = scaler.transform(X_val_flat).reshape(-1, len(comps), len(freqs)).astype(np.float32)

    raw_val = X_val.copy().astype(np.float32)

    train_ds = MilkDataset(X_train_s, y_all[train_idx])
    val_ds = MilkDataset(X_val_s, y_all[val_idx], raw_X_for_save=raw_val)
    ...
    y_true = np.concatenate(all_true)
    y_pred = np.concatenate(all_pred)
    va_loss = total_loss / len(y_true)
    va_acc = accuracy_score(y_true, y_pred)
    va_f1m = f1_score(y_true, y_pred, average="macro")
    ...
    # FLOPs 估算
    dummy = torch.zeros(1, len(comps), len(freqs), dtype=torch.float32, device=device)
    ...
    metrics = {
        ...
        "inference_time_total_sec": float(total_inference_time),
        "inference_time_avg_ms_per_sample": float(avg_inference_time_ms),
    ...
    print(f"[Fold {fold_id}] Inference time: total={total_inference_time*1000:.2f} ms, avg per sample={avg_inference_time_ms:.3f} ms")
    ...
    with torch.no_grad():
        for batch in val_loader:
            xb, yb, rawb = batch
            xb = xb.to(device)
            _, _, h = model(xb)  # h: [B,1, H, W]
            yb_np = yb.numpy()
            rawb_np = rawb.numpy()  # [B, n_comp, n_freq] original
            ...
            for i in range(len(yb_np)):
                ...
                # 转置使高度图变为 [n_freq, n_comp]
                height = rawb_np[i].T               # [n_freq, n_comp]
                nm = height_to_normalmap_np(height)

                sid = f"val_{sample_global:05d}_gt{gt}_pred{int(y_pred[sample_global])}"
                save_normalmap_with_axes(nm, vis_dir / f"{sid}_normal.png", comps, freqs)
                save_surface_3d(height, vis_dir / f"{sid}_surface3d.png", comps, freqs)
...
# ----------------------------
# 9) 主函数：5折交叉验证 + 保存 run/milk_ours
# ----------------------------
def main(
    excel_path: str,
    out_dir: str = "run/milk_ours",
    seed: int = 42,
    epochs: int = 50,
    batch_size: int = 64,
    lr: float = 1e-3,
    ...
    # 仅针对当前Excel的数据格式做兼容：
    # 1) 标签列原始表头可能是 "  kind"，上面已strip为 "kind"
    # 2) 少量数字可能以文本形式存储，例如 "0,00238515"
    # 3) 个别异常值转为NaN后，用同类别中位数补齐，避免训练时报错
    df["kind"] = pd.to_numeric(df["kind"], errors="coerce")
    df = df.dropna(subset=["kind"]).copy().reset_index(drop=True)
    df["kind"] = df["kind"].astype(int)

    feature_cols = [c for c in df.columns if c != "kind"]
    ...
    for c in feature_cols:
        if df[c].isna().any():
            df[c] = df[c].fillna(df.groupby("kind")[c].transform("median"))
            df[c] = df[c].fillna(df[c].median())

    X_all, y_all, comps, freqs, _ = parse_columns_and_build_matrix(df, label_col="kind")
    num_classes = len(np.unique(y_all))
    print(f"Detected {len(comps)} components: {comps}")
    print(f"Detected {len(freqs)} frequencies: {freqs}")
    ...
    inference_times_ms = [m["inference_time_avg_ms_per_sample"] for m in fold_metrics]
    ...
    summary = {
        ...
        "inference_time_avg_ms_per_fold": inference_times_ms,
        "inference_time_mean_ms": float(np.mean(inference_times_ms)),
        "inference_time_std_ms": float(np.std(inference_times_ms)),
    ...
    class_names = [f"class_{c}" for c in range(num_classes)]
    cm_all = confusion_matrix(overall_true, overall_pred, labels=list(range(num_classes)))
    ...
    print(
        "Inference time per sample (mean±std):",
        summary["inference_time_mean_ms"],
        "±",
        summary["inference_time_std_ms"],
        "ms"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--excel_path", type=str, default="D:/1DCNN/Beef-Milk-Juice/data/milk-new/milk-1·.xlsx")
    parser.add_argument("--out_dir", type=str, default="D:/1DCNN/Beef-Milk-Juice/data/milk-new/milk-1_ours")

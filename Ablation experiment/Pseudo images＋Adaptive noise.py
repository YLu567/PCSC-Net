# train_beef_pseudo_adaptivenoise.py
# -*- coding: utf-8 -*-
...
# ----------------------------
# 2) 伪图生成器：8×6 高度图 -> normal map (RGB) (保持不变)
# ----------------------------
...
# ----------------------------
# 3) 自适应噪声模块（保持不变，channels=3 对应伪图RGB）
# ----------------------------
...
# ----------------------------
# 4) 判别网络：仅使用伪图 + 自适应噪声（无 per_comp）
# ----------------------------
class BeefPseudoAdaptiveNoiseNet(nn.Module):
    """
    输入: [B,6,8] (标准化后) -> 转置为高度图 [B,1,8,6] -> 伪图生成 -> 自适应噪声 -> CNN backbone -> 分类
    """
    def __init__(self,
                 num_classes=3,
                 pseudo_size=64,
                 img_feat_dim=128,
                 noise_max_sigma=0.15,
                 dropout=0.2):
        super().__init__()
        self.pseudo = PseudoImageGenerator(out_size=pseudo_size, learnable_refine=True)
        self.noise = AdaptiveNoise(channels=3, max_sigma=noise_max_sigma)
    ...
    def forward(self, x):
        ...
        return:
          ...
          h:      [B,1,8,6] (高度图)
        """
        # 直接从 x 生成高度图（转置并增加通道维度）
        h = x.transpose(1, 2).unsqueeze(1)       # [B,1,8,6]
        img = self.pseudo(h)                      # [B,3,S,S]
        img = self.noise(img)                     # 仅在训练时添加自适应噪声
        feat = self.backbone(img)                 # [B, img_feat_dim]
        logits = self.classifier(feat)            # [B, num_classes]
        return logits, img, h


# ----------------------------
# 5) 可视化：保存伪图(normal map) + 真实3D surface (保持不变)
# ----------------------------
...
def compute_conv_linear_macs_flops(model: nn.Module,
    ...
    """
    用 forward hook 统计 Conv2d/Linear 的 MACs。
...
def train_one_fold(fold_id: int,
    ...
    model = BeefPseudoAdaptiveNoiseNet(
        num_classes=3,
        pseudo_size=pseudo_size,
        img_feat_dim=img_feat_dim,
        noise_max_sigma=noise_max_sigma,
        dropout=dropout
    ).to(device)
...
# ----------------------------
# 7) 主函数：5折交叉验证 + 保存 results/beef-Pseudo images＋ Adaption noise
# ----------------------------
def main(
    excel_path: str,
    out_dir: str = "results/beef-Pseudo images＋ Adaption noise",
    seed: int = 42,
    epochs: int = 50,
    batch_size: int = 64,
    lr: float = 1e-3,
    ...
    config = {
        ...
        "model": "BeefPseudoAdaptiveNoiseNet (pseudo image + adaptive noise, no per-component conv)",
    }
...
if __name__ == "__main__":
    ...
    parser.add_argument("--out_dir", type=str, default="D:/1DCNN/Beef-Milk-Juice/results/beef-Pseudo images＋ Adaption noise")
    ...
    parser.add_argument("--noise_max_sigma", type=float, default=0.15, help="自适应噪声的最大标准差")
    parser.add_argument("--dropout", type=float, default=0.2, help="分类器dropout率")
    parser.add_argument("--val_noise_std", type=float, default=0.1, help="验证时添加的外部高斯噪声标准差")
    ...
    main(
        ...
        noise_max_sigma=args.noise_max_sigma,

...
# ----------------------------
# 3) 简化模型：仅使用伪图分支（无 per_comp、无自适应噪声）
# ----------------------------
class BeefPseudoNet(nn.Module):
    """
    输入原始谱数据 x [B,6,8] -> 转置为高度图 [B,1,8,6] -> 伪图生成 -> CNN backbone -> 分类
    """
    def __init__(self,
                 num_classes=3,
                 pseudo_size=64,
...
# ----------------------------
# 4) 可视化：保存伪图(normal map) + 真实3D surface (保持不变)
# ----------------------------
...
# ----------------------------
# 6) 主函数：5折交叉验证 + 保存 results/beef-pseudo images
# ----------------------------
def main(
    excel_path: str,
    out_dir: str = "results/beef-pseudo images",
    seed: int = 42,
    epochs: int = 50,
    batch_size: int = 64,
    lr: float = 1e-3,
    ...
    config = {
        ...
        "model": "BeefPseudoNet (pseudo image only)",
    }
...
if __name__ == "__main__":
    ...
    parser.add_argument("--out_dir", type=str, default="D:/1DCNN/Beef-Milk-Juice/results/beef-pseudo images")
    ...
    parser.add_argument("--val_noise_std", type=float, default=0.26, help="验证时添加的高斯噪声标准差")

# train_beef_conv_adaptivenoise.py
# -*- coding: utf-8 -*-
...
# ----------------------------
# 3) 自适应噪声模块（修改为支持单通道输入）
# ----------------------------
class AdaptiveNoise(nn.Module):
    def __init__(self, channels=1, max_sigma=0.15):
        super().__init__()
...
# ----------------------------
# 4) 判别网络：仅使用 per_conv + adaptive noise (无伪图)
# ----------------------------
class BeefConvAdaptiveNoiseNet(nn.Module):
    """
    x -> per_comp -> [B,6,8] -> unsqueeze(1) -> [B,1,6,8] -> AdaptiveNoise -> classifier
    """
    def __init__(self,
                 num_classes=3,
        ...
        self.noise = AdaptiveNoise(channels=1, max_sigma=noise_max_sigma)

        # 分类器：将 [B,1,6,8] 展平为 [B,48] 后分类
        self.classifier = nn.Sequential(
            nn.Flatten(),
            nn.Dropout(p=dropout),
            nn.Linear(6 * 8, num_classes)   # 48 -> 3
        )
...
def train_one_fold(fold_id: int,
    ...
    model = BeefConvAdaptiveNoiseNet(
        num_classes=3,
...
# ----------------------------
# 7) 主函数：5折交叉验证 + 保存 results/beef-Conv＋Adaptive noise
# ----------------------------
def main(
    excel_path: str,
    out_dir: str = "results/beef-Conv＋Adaptive noise",
    seed: int = 42,
    epochs: int = 50,
    batch_size: int = 64,
    lr: float = 1e-3,
    ...
    config = {
        ...
        "model": "BeefConvAdaptiveNoiseNet (per-component conv + adaptive noise, no pseudo image)",
    }
...
if __name__ == "__main__":
    ...
    parser.add_argument("--out_dir", type=str, default="D:/1DCNN/Beef-Milk-Juice/results/beef-Conv＋Adaptive noise")
    ...
    parser.add_argument("--val_noise_std", type=float, default=0.2, help="验证时添加的外部高斯噪声标准差")
